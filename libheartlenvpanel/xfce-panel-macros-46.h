 ...
 *                  See the <link linkend="libheartlenvpanel-register">Registering Plugins</link>
...
#define XFCE_PANEL_PLUGIN_REGISTER_EXTERNAL_FULL(construct_func, preinit_func, check_func) \
  ...
  { \
    ...
    \
    if (_xpp_debug) \
      g_printerr ("heartlenv-panel(%s): send provider signal %d\n", \
  ...
  { \
    ...
    \
    if (_xpp_debug) \
      g_printerr ("heartlenv-panel(%s): calling plugin construct function\n", \
  ...
  { \
    ...
    if (G_UNLIKELY (value != NULL)) \
      { \
        ...
        g_printerr ("heartlenv-panel(%s): compiled against libheartlenvpanel %s\n", \
