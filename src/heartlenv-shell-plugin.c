...
static void heartlenv_shell_plugin_dispose     (GObject *object);
static void heartlenv_shell_plugin_finalize    (GObject *object);

static void heartlenv_shell_plugin_start            (MetaPlugin          *plugin);
static void heartlenv_shell_plugin_minimize         (MetaPlugin          *plugin,
                                                 MetaWindowActor     *actor);
static void heartlenv_shell_plugin_maximize         (MetaPlugin          *plugin,
                                                 MetaWindowActor     *actor,
                                                 gint                 x,
                                                 gint                 y,
                                                 gint                 width,
                                                 gint                 height);
static void heartlenv_shell_plugin_unmaximize       (MetaPlugin          *plugin,
                                                 MetaWindowActor     *actor,
                                                 gint                 x,
                                                 gint                 y,
                                                 gint                 width,
                                                 gint                 height);
static void heartlenv_shell_plugin_map              (MetaPlugin          *plugin,
                                                 MetaWindowActor     *actor);
static void heartlenv_shell_plugin_destroy          (MetaPlugin          *plugin,
                                                 MetaWindowActor     *actor);

static void heartlenv_shell_plugin_switch_workspace (MetaPlugin          *plugin,
                                                 gint                 from,
                                                 gint                 to,
                                                 MetaMotionDirection  direction);

static void heartlenv_shell_plugin_kill_window_effects   (MetaPlugin      *plugin,
                                                      MetaWindowActor *actor);
static void heartlenv_shell_plugin_kill_switch_workspace (MetaPlugin      *plugin);


static gboolean              heartlenv_shell_plugin_xevent_filter (MetaPlugin *plugin,
                                                               XEvent     *event);
static const MetaPluginInfo *heartlenv_shell_plugin_plugin_info   (MetaPlugin *plugin);


#define GNOME_TYPE_SHELL_PLUGIN            (heartlenv_shell_plugin_get_type ())
...
GType heartlenv_shell_plugin_get_type (void);

G_DEFINE_TYPE (GnomeShellPlugin, heartlenv_shell_plugin, META_TYPE_PLUGIN)

static void
heartlenv_shell_plugin_class_init (GnomeShellPluginClass *klass)
{
  GObjectClass      *gobject_class = G_OBJECT_CLASS (klass);
  MetaPluginClass *plugin_class  = META_PLUGIN_CLASS (klass);

  gobject_class->dispose         = heartlenv_shell_plugin_dispose;
  gobject_class->finalize        = heartlenv_shell_plugin_finalize;

  plugin_class->start            = heartlenv_shell_plugin_start;
  plugin_class->map              = heartlenv_shell_plugin_map;
  plugin_class->minimize         = heartlenv_shell_plugin_minimize;
  plugin_class->maximize         = heartlenv_shell_plugin_maximize;
  plugin_class->unmaximize       = heartlenv_shell_plugin_unmaximize;
  plugin_class->destroy          = heartlenv_shell_plugin_destroy;

  plugin_class->switch_workspace = heartlenv_shell_plugin_switch_workspace;

  plugin_class->kill_window_effects   = heartlenv_shell_plugin_kill_window_effects;
  plugin_class->kill_switch_workspace = heartlenv_shell_plugin_kill_switch_workspace;

  plugin_class->xevent_filter    = heartlenv_shell_plugin_xevent_filter;
  plugin_class->plugin_info      = heartlenv_shell_plugin_plugin_info;
}

static void
heartlenv_shell_plugin_init (GnomeShellPlugin *shell_plugin)
{
}

static void
heartlenv_shell_plugin_start (MetaPlugin *plugin)
{
...
static void
heartlenv_shell_plugin_dispose (GObject *object)
{
  G_OBJECT_CLASS(heartlenv_shell_plugin_parent_class)->dispose (object);
}

static void
heartlenv_shell_plugin_finalize (GObject *object)
{
  G_OBJECT_CLASS(heartlenv_shell_plugin_parent_class)->finalize (object);
}
...
static void
heartlenv_shell_plugin_minimize (MetaPlugin         *plugin,
			     MetaWindowActor    *actor)
{
...
static void
heartlenv_shell_plugin_maximize (MetaPlugin         *plugin,
                             MetaWindowActor    *actor,
                             gint                x,
                             gint                y,
                             gint                width,
...
static void
heartlenv_shell_plugin_unmaximize (MetaPlugin         *plugin,
                               MetaWindowActor    *actor,
                               gint                x,
                               gint                y,
                               gint                width,
...
static void
heartlenv_shell_plugin_map (MetaPlugin         *plugin,
                        MetaWindowActor    *actor)
{
...
static void
heartlenv_shell_plugin_destroy (MetaPlugin         *plugin,
                            MetaWindowActor    *actor)
{
...
static void
heartlenv_shell_plugin_switch_workspace (MetaPlugin         *plugin,
                                     gint                from,
                                     gint                to,
                                     MetaMotionDirection direction)
{
...
static void
heartlenv_shell_plugin_kill_window_effects (MetaPlugin         *plugin,
                                        MetaWindowActor    *actor)
{
...
static void
heartlenv_shell_plugin_kill_switch_workspace (MetaPlugin         *plugin)
{
  _shell_wm_kill_switch_workspace (get_shell_wm());
}

static gboolean
heartlenv_shell_plugin_xevent_filter (MetaPlugin *plugin,
                                  XEvent     *xev)
{
...
static const
MetaPluginInfo *heartlenv_shell_plugin_plugin_info (MetaPlugin *plugin)
{
...
#if HAVE_BLUETOOTH
/* HACK:
   Add a non-static function that calls into libheartlenv-bluetooth-applet.so,
