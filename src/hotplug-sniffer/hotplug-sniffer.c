...
#define BUS_NAME "org.heartlenv.Shell.HotplugSniffer"
#define AUTOQUIT_TIMEOUT 5

static const gchar introspection_xml[] =
  "<node>"
  "  <interface name='org.heartlenv.Shell.HotplugSniffer'>"
                 ...
                 gpointer user_data)
{
  ...
  g_dbus_connection_register_object (connection,
                                     "/org/heartlenv/Shell/HotplugSniffer",
...
print_debug (const gchar *format, ...)
{
  ...
  g_print ("heartlenv-shell-hotplug-sniffer[%d]: %s.%03d: %s\n", pid, timebuf, (gint) (now.tv_usec / 1000), s);
  g_free (s);
 out:
  ;
}
