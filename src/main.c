...
extern GType heartlenv_shell_plugin_get_type (void);

#define SHELL_DBUS_SERVICE "org.heartlenv.Shell"
#define MAGNIFIER_DBUS_SERVICE "org.heartlenv.Magnifier"

#define OVERRIDES_SCHEMA "org.heartlenv.shell.overrides"
...
shell_dbus_init (gboolean replace)
{
  ...
  shell_dbus_acquire_names (bus,
                            request_name_flags,
  /* Also grab org.heartlenv.Panel to replace any existing panel process */
                            "org.heartlenv.Panel", TRUE,
  /* ...and the org.heartlenv.Magnifier service. */
  ...
  shell_dbus_acquire_name (bus,
                           ...
                           &request_name_result,
                           "org.heartlenv.Caribou.Keyboard", FALSE);
  g_object_unref (bus);
...
GOptionEntry heartlenv_shell_options[] = {
  {
...
main (int argc, char **argv)
{
  ...
  g_option_context_add_main_entries (ctx, heartlenv_shell_options, GETTEXT_PACKAGE);
  ...
  meta_plugin_type_register (heartlenv_shell_plugin_get_type ());
...
/* HACK:
   Add a dummy function that calls into libheartlenv-shell-js.so to ensure it's
   linked to /usr/bin/heartlenv-shell even when linking with --as-needed.
   This function is never actually called.
   https://bugzilla.heartlenv.org/show_bug.cgi?id=670477
*/
