 ...
 *
 * Based on code from heartlenv-panel's clock-applet, file calendar-client.c, with Authors:
 *
...
#define BUS_NAME "org.heartlenv.Shell.CalendarServer"

static const gchar introspection_xml[] =
  "<node>"
  "  <interface name='org.heartlenv.Shell.CalendarServer'>"
...
on_app_schedule_changed_cb (gpointer user_data)
{
  ...
  g_dbus_connection_emit_signal (app->connection,
                                 ...
                                 "/org/heartlenv/Shell/CalendarServer",
                                 "org.heartlenv.Shell.CalendarServer",
                                 "Changed",
                                 NULL, /* no params */
                                 NULL);
      ...
      if (until < since)
        {
          g_dbus_method_invocation_return_dbus_error (invocation,
                                                      "org.heartlenv.Shell.CalendarServer.Error.Failed",
...
on_name_lost (GDBusConnection *connection,
              ...
              gpointer         user_data)
{
  g_print ("heartlenv-shell-calendar-server[%d]: Lost (or failed to acquire) the name " BUS_NAME " - exiting\n",
           (gint) getpid ());
  gtk_main_quit ();
}
                       ...
                       gpointer data)
{
  if (condition & G_IO_HUP)
    {
      g_debug ("heartlenv-shell-calendar-server[%d]: Got HUP on stdin - exiting\n",
               (gint) getpid ());
      gtk_main_quit ();
    }
  else
      ...
      char **argv)
{
  ...
  opt_context = g_option_context_new ("heartlenv-shell calendar server");
...
print_debug (const gchar *format, ...)
{
  ...
  g_print ("heartlenv-shell-calendar-server[%d]: %s.%03d: %s\n", pid, timebuf, (gint) (now.tv_usec / 1000), s);
  g_free (s);
 out:
  ;
}
