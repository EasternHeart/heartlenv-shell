...
struct _ShellGlobal {
   ...
   * http://bugzilla.heartlenv.org/show_bug.cgi?id=570641
   * 
...
shell_global_init (ShellGlobal *global)
{
  ...
  global->userdatadir = g_build_filename (g_get_user_data_dir (), "heartlenv-shell", NULL);
  g_mkdir_with_parents (global->userdatadir, 0700);

  global->settings = g_settings_new ("org.heartlenv.shell");
  ...
  ca_context_change_props (global->sound_context, CA_PROP_APPLICATION_NAME, PACKAGE_NAME, CA_PROP_APPLICATION_ID, "org.heartlenv.Shell", NULL);
...
shell_global_class_init (ShellGlobalClass *klass)
{
  ...
  /* Emitted from heartlenv-shell-plugin.c during event handling */
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_object ("settings",
                                                        "Settings",
                                                        "GSettings instance for heartlenv-shell configuration",
                                                        G_TYPE_SETTINGS,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_DATADIR,
                                   g_param_spec_string ("datadir",
                                                        "Data directory",
                                                        "Directory containing heartlenv-shell data files",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_IMAGEDIR,
                                   g_param_spec_string ("imagedir",
                                                        "Image directory",
                                                        "Directory containing heartlenv-shell image files",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_USERDATADIR,
                                   g_param_spec_string ("userdatadir",
                                                        "User data directory",
                                                        "Directory containing heartlenv-shell user data",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_FOCUS_MANAGER,
 ...
 *   GDK filter function
 *     => Mutter
 *     => heartlenv_shell_plugin_xevent_filter()
 ...
 */
static void
heartlenv_shell_gdk_event_handler (GdkEvent *event_gdk,
                               gpointer  data)
{
                          ...
                          MetaPlugin  *plugin)
{
  ...
  gdk_event_handler_set (heartlenv_shell_gdk_event_handler, global->stage, NULL);
 ...
 *
 * http://bugzilla.heartlenv.org/show_bug.cgi?id=469231
 * http://bugzilla.heartlenv.org/show_bug.cgi?id=357585
 */
 ...
 * https://bugzilla.heartlenv.org/show_bug.cgi?id=597292.
 */
void
 ...
 * @global: The #ShellGlobal.
 *
 * Launch the heartlenv-shell-calendar-server helper.
 */
void
shell_global_launch_calendar_server (ShellGlobal *global)
{
  ...
  if (bin_dir != NULL)
    calendar_server_exe = g_strdup_printf ("%s/heartlenv-shell-calendar-server", bin_dir);
  else
    calendar_server_exe = g_strdup_printf (GNOME_SHELL_LIBEXECDIR "/heartlenv-shell-calendar-server");
  ...
  /* Note that heartlenv-shell-calendar-server exits whenever its stdin
