...
const Indicator = new Lang.Class({
    ...
    _init: function() {
        ...
        this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        this.menu.addSettingsAction(_("Sound Settings"), 'heartlenv-sound-panel.desktop');
    ...
    _maybeShowInput: function() {
        ...
        if (this._input && recordingApps) {
            for (let i = 0; i < recordingApps.length; i++) {
                ...
                // but skip heartlenv-volume-control and pavucontrol
                // (that appear as recording because they show the input level)
                if (!id || (id != 'org.heartlenv.VolumeControl' && id != 'org.PulseAudio.pavucontrol')) {
                    showInput = true;
                    break;
                }
            }
