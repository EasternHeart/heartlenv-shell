...
export TEXTDOMAIN="heartlenv-panel"
...
case "$1" in
  -h|--help)
    ...
    ;;
  -V|--version)
    exec @bindir@/heartlenv-panel -V "$(basename $0)"
    exit 0
    ;;
  -p|--pointer)
    ATPOINTER="true"
...
exec @bindir@/heartlenv-panel --plugin-event=applicationsmenu:popup:bool:$ATPOINTER
